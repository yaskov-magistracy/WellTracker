...
export function createKcalStatisticBarChartOptions(): EChartsCoreOption {
  ...
  const dates = calorieData.map(d => d.date);
  const values = calorieData.map(d => d.value);

  const minValue = Math.min(...values.filter(v => v != null)) - 100;
  const maxValue = Math.max(...values.filter(v => v != null)) + 100;
